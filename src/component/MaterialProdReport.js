...
const MaterialProductionReport = () => {
    ...
    const getTotalProducedQty = (val) => {
        const totalProducedQty = getFilteredData().reduce((accm, value) => {
            return val === 1 ? accm + value['Available Qty 1'] : accm + value['Available Qty 2']
        }, 0);
    ...
    const getTotalConsumedQty = (val) => {
        const totalConsumedQty = getFilteredData().reduce((accm, value) => {
            return val === 1 ? accm + value['Consumed Qty 1'] : accm + value['Consumed Qty 2'];
        }, 0);
    ...
    const getTotalBalanceQty = (val) => {
        const totalBalanceQty = getFilteredData().reduce((accm, value) => {
            return val === 1 ? accm + value['Balance Qty 1'] : accm + value['Balance Qty 2'];
        }, 0);
    ...
    return (
        <div>
            <div className="d-flex justify-content-between">
                ...
                {/* <div className="d-flex justify-content-end"> <button style={{ textDecoration: 'underline', color: 'blue', border: "none", background: "none" }} onClick=""> Download Report</button> </div> */}
            </div>
                    ...
                    >
                        <table className={"table table-bordered table-striped"}>
                            <thead className="sticky-top" >
                                <tr>
                                    <th>Date</th>
                                    <th style={{ width: '100px', whiteSpace: "nowrap", fontSize: "11px" }}>
                                        {itemCodeSearchVisible ? (
                                            <div className="search-input-container" style={{ position: 'relative', top: '0px', left: '0px', backgroundColor: 'white' }}>
                                                <TextField
                                                    type="text"
                                                    variant="outlined"
                                                ...
                                                />
                                                <span className="clear-button" style={{ position: 'absolute' }} onClick={(e) => {
                                                ...
                                                <span className="search-icon-button" style={{ marginLeft: "10px" }}>
                                                    <FaSistrix onClick={(e) => setItemCodeSearchVisible(!itemCodeSearchVisible)} />
                                                </span>
                                            </>)}
                                    </th>
                                    <th style={{ width: '120px', whiteSpace: "nowrap", fontSize: "11px" }}>
                                        {itemDescSearchVisible ? (
                                            <div className="search-input-container" style={{ position: 'relative', top: '0px', backgroundColor: 'white' }}>
                                                <TextField
                                                    type="text"
                                                    variant="outlined"
                                    ...
                                    <th style={{ fontSize: "11px", whiteSpace: "nowrap" }}>Job Id</th>
                                    {/* <th style={{ fontSize: "11px", whiteSpace: "nowrap" }}>Material ID</th> */}
                                    <th style={{ width: '130px', whiteSpace: "nowrap", fontSize: "11px" }}>{nodeDescSearchVisible ? (
                                    ...
                                    <th style={{ fontSize: "11px", whiteSpace: "nowrap" }}>Batch Id</th>
                                    <th style={{ fontSize: "11px", whiteSpace: "nowrap" }}>Available (Meters)</th>
                                    <th style={{ fontSize: "11px", whiteSpace: "nowrap" }}>Consumed (Meters)</th>
                                    <th style={{ fontSize: "11px", whiteSpace: "nowrap" }}>Balance (Meters)</th>
                                    {/* <th>Units</th> */}
                                    <th style={{ fontSize: "11px", whiteSpace: "nowrap" }}>Available (Kgs)</th>
                                    <th style={{ fontSize: "11px", whiteSpace: "nowrap" }}>Consumed (Kgs)</th>
                                    <th style={{ fontSize: "11px", whiteSpace: "nowrap" }}>Balance (Kgs)</th>
                                    {/* <th>Units</th> */}
                                </tr>
                            </thead>
            ...
            <div className="col-12">
                            <h6>Total Produced qty (m): {getTotalProducedQty(1)?.toFixed(2)}, Total Produced qty (kgs): {getTotalProducedQty(2)?.toFixed(2)}</h6>
                            <h6>Total Consumed qty (m): {getTotalConsumedQty(1)?.toFixed(2)}, Total Consumed qty (kgs): {getTotalConsumedQty(2)?.toFixed(2)}</h6>
                            <h6>Total Balance qty (m): {getTotalBalanceQty(1)?.toFixed(2)}, Total Balance qty (kgs): {getTotalBalanceQty(2)?.toFixed(2)}</h6>
                        </div>
        </div>
