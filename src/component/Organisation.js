...
function Organisation() {
  ...
  useEffect(() => {
    const description = data.filter(item => item.id == materialTypeId)
    .map(item => item.typeDescription)
    const MaterialcatId = data.filter(item => item.id == materialTypeId)
    .map(item => item.materialCategoryId)
    const routeId = data.filter(item => item.id == materialTypeId)
    .map(item => item.routeId)
    const specification = data.filter(item => item.id == materialTypeId)
    ...
    <div style={{ width: "100%", height: "100%" }}>
      ...
      <BootstrapDialog
        ...
        <div className="col-12 p-2">
          ...
          {/* <div className="offset-0 row mt-2">
            <div className="col-2">
              <label htmlFor="MaterialCategoryID">Material Category ID:</label>
            </div>
            <div className="col-4">
              <input
                type="number"
        ...
        <form>
          ...
          <div className="offset-0 row mt-1">
            <div className="col-2">
