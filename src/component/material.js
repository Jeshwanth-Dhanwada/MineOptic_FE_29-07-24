...
function Material() {
  ...
  useEffect(() => {
    const name = data.filter(item => item.materialId == materialId)
    .map(item => item.materialName)
    const unitId = data.filter(item => item.materialId == materialId)
              ...
              <Table sx={{ minWidth: 465 }} aria-label="simple table">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell style={{fontSize:'11px'}}>Material Unit Id</TableCell>
      ...
      <form onSubmit={handleSubmit} style={{fontSize:'11px'}}>
        ...
        >
          ...
          <div className="offset-0 row">
            ...
            &nbsp;
            <a onClick={Emptyfields} className="btn btn-danger btn-sm" type="submit">
              <FaXmark />
            </a>
            </div>
          </div>
